...
public class NachoConfig {
    ...
    public static void init(File configFile) {
        ...
        try {
//            System.out.println("Loading NachoSpigot config from " + configFile.getName()); // FeatherMC - removed useless logs
            config.load(CONFIG_FILE);
        } catch (IOException ignored) {
        } catch (InvalidConfigurationException ex) {
            LOGGER.log(Level.ERROR, "Khata Dar Load Shodane nacho.yml, Lotfan Config Ra Barresy Konid ", ex);
...
//        c.setHeader(HEADER); // FeatherMC
    ...
    private static void serverBrandName() {
        serverBrandName = getString("settings.brand-name", "FeatherMC");
    }
